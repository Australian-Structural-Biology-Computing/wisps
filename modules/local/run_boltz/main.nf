...
process RUN_BOLTZ {
    ...
    tuple val(meta), path ("${meta.id}_plddt.tsv")                              , optional: true, emit: multiqc
    // TODO: support cif as well like with HelixFold3
    tuple val(meta), path ("${meta.id}_boltz.pdb")                              , optional: true, emit: top_ranked_pdb
    ...
    tuple val(meta), path ("${meta.id}_boltz_msa.tsv")                          , optional: true, emit: msa_raw
    tuple val(meta), path ("${meta.id}_*_pae.tsv")                              , optional: true, emit: pae_raw
    tuple val(meta), path ("${meta.id}_ptm.tsv")                                , optional: true, emit: ptm_raw
