...
1. The default behavior is for `--mode all-all`. In this case, the workflow will generate interaction pairs for all samples in the sample sheet against each other (all possible combinations) and keep the unique pairs after ignoring the order of the samples. This does not require the group column to be provided in the sample sheet, and this is the default behavior of the workflow unless the `--mode` parameter is customised. When using this mode, the user can liit the number of formed interactions through the parameter `interaction_threshold` which takes a numeric value. by default the value is 0 and all interactions are formed. For any positive value `n`, onlye the neighboring paires that are within `n` neighbors will be considered. In this case, teh order of the samples in teh sample sheet matters. For example, if `--interaction_threshold 2`, the 2 nighbouring samples (2 before and 2 after) will be consdered.
